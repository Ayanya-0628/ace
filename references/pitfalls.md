...
| 人口学变量编码标签凭空推断 | 客户只给数字编码(1,2,3,4)无编码对照，AI从参考论文或常识猜测标签（如3=分诊台）| **铁律：无编码对照一律输出数字（选项1/选项2），不得自行推断标签。** 参考论文的人口学编码≠客户问卷编码 [A177] |
...
| 客户质疑"分诊台数据哪来的" | 参考论文的人口学编码≠客户问卷，AI自行推断后客户发现标签与实际不符 | **开工前必须确认：客户是否提供了编码对照表？没有则①问客户要②一律输出数字。** 参考论文/模板只能参考分析方法和表格格式，人口学变量选项必须来自客户 [A177] |
...
# ══════ 无编码对照时的处理规则 [A177踩坑] ══════
# 🔴 如果客户没有提供编码对照表（即不知道1=什么、2=什么）：
#    ① 优先：向客户索要编码对照表
#    ② 兜底：一律使用数字标签（选项1/选项2/选项3...），绝不从参考论文或常识推断
#    ③ 禁止：从参考论文的人口学变量照搬编码（论文的人口学变量是论文作者自己设计的，不等于客户的）
# 🔴 参考论文/模板只能参考：分析方法、表格格式、量表计分方式
# 🔴 参考论文/模板不能参考：人口学变量选项内容、编码含义
