...
## 7.1 单因素分析表默认排版（医学/护理常用）

```python
headers = ['变量', '组别', '例数', dv_name, 't/F', 'P']
rows = []
for var_name, stat_val, p_val, group_stats in results:
    first = True
    for gs in group_stats:
        rows.append([
            var_name if first else '',
            gs['label'],
            str(gs['n']),
            f"{gs['mean']:.2f}±{gs['sd']:.2f}",
            f'{stat_val:.3f}' if first else '',
            f'{p_val:.3f}' if first else '',
        ])
        first = False
```

适用规则：
- 适用于单一核心因变量的 t 检验 / 单因素 ANOVA 表
- 变量名只在首行出现一次，后续组别行留空
- 统计量与 P 值只在首行显示
- 默认样式参考医学论文常见排版：`变量 | 组别 | 例数 | 工作负荷/得分 | t/F | P`
