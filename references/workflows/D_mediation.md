...
Step 4  中介效应检验
        ...
        │   └─ CI 不含 0 → 间接效应显著
        ├─ 效应分解表（论文/客户版默认格式）：
        │   ├─ 表头：效应 | Effect | SE | 95%CI
        │   ├─ 总效应 c：Effect、SE、95%CI 填回归估计结果
        │   ├─ 直接效应 c'：Effect、SE、95%CI 填回归估计结果
        │   ├─ 间接效应 ab：Effect=a×b，SE与95%CI填Bootstrap结果
        │   └─ 表注说明：总效应和直接效应为回归估计；间接效应基于Bootstrap重复抽样
        ├─ 中介类型判断：
        │   ├─ 间接效应显著 + c'显著 → 部分中介
...
Step 7  Word 输出：逐步回归表 + 效应分解与Bootstrap检验表 + 调节效应图
```
