...
踩坑记录:
    ...
    - OMS: 在 SPSS 27 + statisticspython3.bat 路线下更稳，回归表不会出现“更高版本创建”的伪兼容提示
    - SpssClient + OUTPUT SAVE: 某些环境下虽然能导出中文 SPV，但回归类对象在本机 Viewer 中可能显示为“无法显示这个表”
    - stats.exe -production silent: 可运行语法但无法控制输出保存
    - stats.com -f -type -out: SPSS 27 不支持这些参数

推荐方案:
    默认优先 OMS（兼容性最稳） → 仅在明确需要中文 GUI 风格输出且已实测正常时再尝试 SpssClient
"""
import os
import sys
