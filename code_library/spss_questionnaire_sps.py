...
【语法结构铁律】
  ...
  - 手动在 SPSS GUI 中直接运行 `.sps` 时，结尾可保留 `OUTPUT SAVE OUTFILE='绝对路径.spv'.`
  - 若通过 SPSS Python 接口批量生成 `.spv`，默认改用 `OMS /DESTINATION FORMAT=SPV ...`，避免回归表出现伪“高版本创建”兼容提示
...
def generate_questionnaire_sps(cfg: QuestionnaireConfig) -> str:
    ...
    6. 回归分析（REGRESSION）
    7. 输出保存（默认写入 OUTPUT SAVE，供 GUI 手动运行）
