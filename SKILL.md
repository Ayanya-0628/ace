...
| 覆盖了已交付的文件 | 重跑脚本直接输出到同路径 | 先备份再跑（S13），或输出路径加日期后缀 |

### 19.6 Word 报告踩坑

| 坑 | 原因 | 解决方案 |
|----|------|--------|
| 表格内段落行距太大 | 表格段落继承了正文1.5倍行距样式 | 表格单元格必须单独设：单倍行距+段前段后0+无缩进（S6.2） |
...
| 坑 | 原因 | 解决方案 |
|----|------|--------|
| SPSS 无头模式报 NullPointerException | SPSS 27 Java 组件 Bug | 禁止后台调SPSS，生成 .sps 语法由用户手动运行（S11） |
| 中文路径 Python 报编码错误 | Windows 默认 GBK | PowerShell 中加 $env:PYTHONUTF8="1" |
| savReaderWriter 读 .sav 乱码 | 文件编码问题 | 先尝试，失败则用 SPSS 导出为 Excel 再读 |
